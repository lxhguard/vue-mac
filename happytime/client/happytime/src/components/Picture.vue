<template>
  <div class="picture">
    ...
    <div class="item" v-for="(picture,index) in pictures" :key="index" v-else>
      <div class="author_xinxi">
          <img class="author_touxiang" :src="picture.profile_image" alt="">
    ...
    <div class="pageList">
      ...
      <ul>
        <li v-for="(count,index) in 10" :key="index" @click="getData()" >
          <router-link :to="{name:'Index',params:{type:3,page:count}}">
            {{count++}}
          </router-link>
        </li>
      </ul>
...
export default {
  ...
  beforeCreate:function(){
      axios.get('https://www.apiopen.top/satinApi?type=3&page=1')
      .then(response=>{
         this.pictures=response.data.data;
         this.loading=false;
      })
